...
module.exports = {
	...
	execute: async (bot, msg, args, cfg) => {
		...
		try {
			await bot.send(msg.channel, "Warning: This will overwrite your data. Only use this command with a recent, untampered .json file generated from the export command from either me ,PluralKit or TupperBox. Please reply 'yes' if you wish to continue.");
		...
		if(data.tuppers) { //tupperbox file
			if(data.tuppers.length > 3000 || data.groups.length > 500) {
				return "Data too large for import. Please visit the support server for assistance: https://discord.gg/kaZmvvJ";
			}
			...
			try {
				...
				for(let i=0; i<groups.length; i++) {
					...
					if(!old.name) { //update existing entry
						added++;
						await bot.db.addGroup(uid,g.name, client);
					} else updated++;
